...
| `podSelectors` | `[]LabelSelector` | `[]` (all pods) | Restricts eviction candidates to pods matching at least one selector. An empty list means all evictable pods are candidates. |
...
- 2026-08-08: Optimized standard deviation recalculation to O(1) per pod, removing the O(pods²) per-node cost limitation.
